...
def build_post(src, bs, owned_ids, out_dir, force=False, pool_ids=None, gw=None):
    ...
    gw=N (back-fill, 7 Sep 2026): build the same snapshot for an earlier finished GW; main() then writes only pitchapi_gw{N}.json / shots_gw{N}.json."""
    cov, warn = {}, {}
    ev = bs["events"]; cur = next((e for e in ev if e["id"] == gw), None) if gw else next((e for e in ev if e["is_current"]), None)
    if not cur:
        return None, "no current event" if not gw else f"GW{gw} not found"
    L = cur["id"]
    ...
    if not gw and not force and not src.offline and glob.glob(os.path.join(out_dir, f"GW{L}_POST_*.json")):
...
def main():
    ...
    ap.add_argument("--gw", type=int, help="POST back-fill: build GW N and write only pitchapi_gw{N}.json / shots_gw{N}.json (no POST snapshot, latest_post untouched)")
    ...
    if a.mode == "post":
        ...
        if a.gw:   # back-fill: side files only
